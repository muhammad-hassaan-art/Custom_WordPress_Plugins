...
function process_impact_data($clinic_submission_id, $impact_form_id) {
    ...
    return integrate_impact_form_data($impact_data);
}


/* Map Impact Form data in ACF Fields of Clinics */

function integrate_impact_form_data($impact_form_data) {
    
    $clinic_impact_data = array(
        'year' => $impact_form_data['Year'],
        'clinic_impact_years' => array(),
    );
    foreach ($impact_form_data as $year => $year_data) {
        if ($year === 'Year') {
            continue;
        }
